...
class HayaScraper(SeleniumBaseScraper):
    ...
    def parse_listing_list(self, html: str) -> List[Dict[str, Any]]:
        ...
        # Haya usa tarjetas de propiedades
        items = soup.find_all('div', class_=re.compile(r'property-card|inmueble-card|asset'))

        if not items:
            items = soup.select('.card, .listing-item, .resultado-item')

        self.logger.debug(f"Haya: Found {len(items)} items")
    ...
    def _parse_listing_item(self, item: BeautifulSoup) -> Dict[str, Any]:
        ...
        # URL
        link = item.find('a', href=re.compile(r'/inmueble|/vivienda|/activo'))
        if not link:
            link = item.find('a')
        ...
        if not title_elem:
            title_elem = item.find(class_=re.compile(r'title|titulo|nombre'))
        if title_elem:
        ...
        # Características
        features = item.find_all(class_=re.compile(r'feature|caracteristica|detalle'))
